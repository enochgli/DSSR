...
class pointwhCoder(pointCoder):
    ...
    def decode(self, rel_codes):
        # print ('xyxy decoding')
        boxes = self.anchor
        pixel_y = 1. / self.patch_count[0]
        pixel_x = 1. / self.patch_count[1]
    ...
    def meshgrid(self, boxes):
        ...
        xs = torch.nn.functional.interpolate(xs, size=self.patch_pixel[1], mode='linear', align_corners=True)
        ys = torch.nn.functional.interpolate(ys, size=self.patch_pixel[0], mode='linear', align_corners=True)
        xs, ys = xs.unsqueeze(3).repeat_interleave(self.patch_pixel[1], dim=3), ys.unsqueeze(2).repeat_interleave(
            self.patch_pixel[0], dim=2)
        results = torch.stack([xs, ys], dim=-1)
        results = results.reshape(B, self.patch_count[0] * self.patch_count[1] * self.patch_pixel[0] * self.patch_pixel[1], 2)
        return results

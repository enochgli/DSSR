    ...
    def forward(self, feat_left: torch.Tensor, feat_right: torch.Tensor, pos_enc: Optional[Tensor] = None):
        ...
        if pos_enc is not None:
            with torch.no_grad():
                ...
                indexes_r = torch.linspace(w - 1, 0, w).repeat(bs, 1).view(bs, w, 1).to(feat_left.device)
                indexes_c = torch.linspace(0, w - 1, w).repeat(bs, 1).view(bs, 1, w).to(feat_left.device)
                pos_indexes = (indexes_r + indexes_c).view(bs, -1).long()  # WxW' -> WW'
        else:
            pos_indexes = None
...
class MultiheadAttentionRelative(nn.MultiheadAttention):
    ...
    def forward(self, query, key, value, attn_mask=None, pos_enc=None, pos_indexes=None):
        ...
        if pos_enc is not None:
            # reshape pos_enc
            bs, _, _ = pos_enc.size()
            pos_indexes_tmp = pos_indexes[0, :].squeeze(0)
            pos_enc = torch.index_select(pos_enc, 1, pos_indexes_tmp).view(bs, w, w,   # HxWxC
        ...
        if q_r is not None:
            q_r = q_r.contiguous().view(bs, w, w, self.num_heads, head_dim)  # bsxWxW'xExC
        if k_r is not None:
            k_r = k_r.contiguous().view(bs, w, w, self.num_heads, head_dim)
        ...
        if pos_enc is not None:
            # 0.3 s
            attn_feat_pos = torch.einsum('wnec,wvec->newv', q, k_r[0,:].squeeze(0))  # NxExWxW'
            attn_pos_feat = torch.einsum('vnec,wvec->newv', k, q_r[0,:].squeeze(0))  # NxExWxW'
...
class PositionEncodingSine1DRelative(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x_embed = torch.linspace(w - 1, -w + 1, 2 * w - 1, dtype=torch.float32, device=x.device).repeat(bs, 1)
        ...
        pos_x = x_embed[:, :, None] / dim_t  # bsx2W-1xC
        # interleave cos and sin instead of concatenate
        pos = torch.stack((pos_x[:, :, 0::2].sin(), pos_x[:, :, 1::2].cos()), dim=2).flatten(2)  # 2W-1xC
